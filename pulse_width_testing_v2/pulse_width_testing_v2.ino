...
void setup_time_test(){
  ...
  for (int k = 0; k<=1; k++){ 
    ...
    while (diff > 100){
      ...
      Serial.println();
      Serial.print(F("Setup Time: ")); Serial.print(setup_time); 
      ...
      if (response == 'f') {
        ...
        if (fail_count == 0) {        
          ...
          setup_time = setup_time + diff*1.3;
          diff = diff/10;
          fail_count++;
        }
        else{
          setup_time = setup_time + diff*5;
          fail_count++;
        }
      }
      else if (response == 'r'){
        ...
        Serial.println(F("Type new setup time (0 - 100000)"));
        serialFlush();
        while(Serial.available()==0);
        long new_ts = Serial.parseInt();
        if (new_ts > 0 && new_ts < 200000){
          setup_time = (unsigned long) new_ts;
        }
        else{
          Serial.println(F("Setup Time unchanged. Try again."));
        }
        serialFlush();
        // do nothing, just run again
      }
        ...
        last_working_ts = setup_time;
        setup_time = setup_time - diff;
        fail_count = 0;
      }
    ...
    Serial.print("_ts"); Serial.println(last_working_ts);
    Serial.println();
    ...
    for (int i=5; i>=0; i--){
      ...
      setup_time = last_working_ts + i*10000/5;
      run_ts_test();
      reset(1);
    }
    ...
    while(test_done==0){
      ...
      for (int i=5; i>=0; i--){
        Serial.println(i);
        setup_time = last_working_ts + i*100000/5;
        run_ts_test();
        if (i!=0) reset(1);
      }
      delay(2000);
...
void cycle(unsigned long pw){
  unsigned long temp_pulse_width = pulse_width;
  for (int i=5; i>=0; i--){
    
    pulse_width = pw + i*10000/5;
    run_pw_test();
    reset(1);
  }
  pulse_width = temp_pulse_width;
}

void run_ts_test(){
  byte toggle = 0;
  boolean ts_vs_pw = (setup_time < (-1)*pulse_width);
  ...
  while(toggle < 2){
    ...
    // 
    if(elapsed_time >= 500000 && toggle == 0){
      set_valve(row[test_row],0);
      toggle = 1;
    }

    // 
    if(elapsed_time >= 500000 - setup_time){
      set_valve(col[test_col],final_state);
      if (ts_vs_pw) toggle = 2;
    }

    // 
    if(elapsed_time >= 500000 + pulse_width){
       set_valve(row[test_row],1);
       if (!ts_vs_pw) toggle = 2;
    }
